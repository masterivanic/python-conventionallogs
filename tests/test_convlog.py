...
class TestConvLogPy(unittest.TestCase):
    ...
    def test_debug_vars_decorator(self):
        ...
        with patch("sys.stdout", new_callable=io.StringIO) as mock_stdout:
            ...
            output = mock_stdout.getvalue()
            lines = [
                json.loads(line.strip()) for line in output.strip().split("\n") if line
            ]

            self.assertGreaterEqual(len(lines), 2)

            arg_log = next(
                (line for line in lines if "Arguments of function" in line["message"]),
                None,
            )
            ...
            var_log = next(
                (line for line in lines if "Variables of function" in line["message"]),
                None,
            )
